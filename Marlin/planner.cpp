...
void Planner::_buffer_line(const float &a, const float &b, const float &c, const float &e, float fr_mm_s, const uint8_t extruder) {
  ...
  LOOP_XYZE(i) {
    float cs = FABS(current_speed[i] = delta_mm[i] * inverse_mm_s); // This line is different from official RCBugFix: search tag: ARDUINO_ARCH_SAM
  ...
  bool limited = false;
  LOOP_XYZE(i) {
    float jerk = FABS(current_speed[i]); // This line is different from official RCBugFix: search tag: ARDUINO_ARCH_SAM
    if (jerk > max_jerk[i]) {
