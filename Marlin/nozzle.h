...
class Nozzle {
  private:
    ...
    static void zigzag(
      ...
      #if ENABLED(NOZZLE_CLEAN_FEATURE)
        float A = FABS(end.y - start.y); // [twice the] Amplitude // This line is different from official RCBugFix: search tag: __SAM3X8E__
        float P = FABS(end.x - start.x) / (objects << 1); // Period // This line is different from official RCBugFix: search tag: __SAM3X8E__
